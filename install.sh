...
ld_library_path_line="export LD_LIBRARY_PATH=\$LD_LIBRARY_PATH:$current_directory/src"
if ! grep -q "$ld_library_path_line" "$HOME/.bashrc"; then
    echo "$ld_library_path_line" >> "$HOME/.bashrc"
    echo "La variable LD_LIBRARY_PATH a été ajoutée au fichier .bashrc avec le chemin $current_directory/src"
else
